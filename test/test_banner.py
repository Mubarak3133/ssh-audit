...
class TestBanner:
    ...
    def test_simple_banners(self):
        banner = lambda x: self.ssh_banner.parse(x)  # noqa
    ...
    def test_invalid_banners(self):
        b = lambda x: self.ssh_banner.parse(x)  # noqa
    ...
    def test_banners_with_comments(self):
        b = lambda x: self.ssh_banner.parse(x)  # noqa
        assert repr(b('SSH-2.0-OpenSSH_7.2p2 Ubuntu-1')) == '<SSH_Banner(protocol=2.0, software=OpenSSH_7.2p2, comments=Ubuntu-1)>'
        assert repr(b('SSH-1.99-OpenSSH_3.4p1 Debian 1:3.4p1-1.woody.3')) == '<SSH_Banner(protocol=1.99, software=OpenSSH_3.4p1, comments=Debian 1:3.4p1-1.woody.3)>'
        assert repr(b('SSH-1.5-1.3.7 F-SECURE SSH')) == '<SSH_Banner(protocol=1.5, software=1.3.7, comments=F-SECURE SSH)>'
